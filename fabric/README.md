...
Press **O** while on a hotbar slot to lock it in place. While a slot is locked:
...
The lock stays on that slot as you switch hotbar slots around it. Press **O** again while back on the locked slot to unlock it, or switch to a different slot and press **O** to move the lock there instead. An action-bar message confirms every lock/unlock.
