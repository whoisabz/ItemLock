 ...
 *
 * This generation uses Minecraft's stratum layering, so a plain fill() composites on top
 * rather than needing the RenderType.guiOverlay() trick the 1.21 - 1.21.5 variants use.
 * render() is called from GuiMixin, and opens a fresh stratum first so the badge lands
 * above the hotbar items - the Forge overlay event used on 1.21.11 does that implicitly,
 * but a mixin at the tail of Gui.render is still inside the HUD's own stratum.
 */
